        ...
        for i in range(0,dic_size):
            ...
            for item in fieldnames_in:
                ...
                for coderx in coders:
                    ...
                    answer = full_dict['statements_'+coderx][i][item]
                ...
                if item in ABDICO:
                    if others_empty:
                        row[item+'_hi']= 0
                        row[item+'_lo']= 0
                        row[item+'_position'] = 0
                    else:
                        row[item+'_hi']= upper_bound(others) - upper_bound(answers)
                        row[item+'_lo']= maxagreement(others)/len(others) - maxagreement(answers)/len(answers)
                        row[item+'_position'] = posagreement(others, Master[i]['Institutional Statement'], len(coders) - 1) - posagreement(answers, Master[i]['Institutional Statement'], len(coders))
                else:
                    if others_empty:
                        row[item]= 0
                    else:
                        row[item]= others.count(full_dict['statements_'+coder][i][item])/len(coders)
